...
start(_Type, _Args) ->
    case color_filter_sup:start_link() of
        {ok, Pid} ->
...
stop(_State) ->
    catch cowboy:stop_listener(color_filter_query_listener),
    catch em_pop_sup:stop_node(color_filter),
    ok.
...
start_pop_and_http() ->
    PopPort   = application:get_env(color_filter, pop_port,   9414),
    QueryPort = application:get_env(color_filter, query_port, 9415),
    Seeds     = application:get_env(color_filter, pop_seeds,  []),
    Vec = em_filter_vec:from_capabilities(base_capabilities()),
    catch em_pop_sup:stop_node(color_filter),
    catch cowboy:stop_listener(color_filter_query_listener),
    {ok, PopPid} = em_pop_sup:start_node(color_filter, #{
        port            => PopPort,
    ...
    Dispatch = cowboy_router:compile([
        {'_', [{"/agent/query", em_filter_http,
                #{server => color_filter_server}}]}
    ]),
    {ok, _} = cowboy:start_clear(color_filter_query_listener,
                                  [{port, QueryPort}],
                                  #{env => #{dispatch => Dispatch}}),
    logger:notice("[color_filter] gossip port ~w  query port ~w",
                  [PopPort, QueryPort]),
    ok.
